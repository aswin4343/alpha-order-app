...
function exportToCSV(rows) {
  const cols = ['Date','Order Date','Sales Rep','Shop','Route','Product','Price Type','Normal Price','Requested Price','Approved Price','Diff','Diff %','Decision','Reason','Competitor','Other Reason','Decided By','Decided At']
  const lines = [cols.join(',')]
  for (const r of rows) {
    const effectiveApproved = r.approved_price ?? r.requested_price
    const diff = r.normal_price != null && effectiveApproved != null ? (effectiveApproved - r.normal_price).toFixed(2) : ''
    const pct = r.normal_price ? ((effectiveApproved - r.normal_price) / r.normal_price * 100).toFixed(1) + '%' : ''
    lines.push([
      r.created_at?.slice(0,10), r.order_date||'', r.sales_rep_name||'', r.shop_name||'', r.route||'',
      r.product_name||'', r.price_type||'',
      r.normal_price??'', r.requested_price??'', effectiveApproved??'', diff, pct,
      r.decision||'',
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-100 shadow-sm">
          <table className="min-w-full text-[11px] text-left">
            <thead className="bg-slate-50 text-slate-500 uppercase text-[9px] tracking-wide">
              <tr>{['Date','Rep','Shop','Product','Type','Normal','Requested','Approved','Diff','%','Decision','Reason','Details','By'].map(h=>(
            ...
            <tbody className="divide-y divide-slate-100">
              {displayed.map(r=>{
                const effectiveApproved = r.approved_price ?? r.requested_price
                const diff=r.normal_price!=null&&effectiveApproved!=null?effectiveApproved-r.normal_price:null
                ...
                return(
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-purple-700">{fmt(r.requested_price)}</td>
                    <td className={`px-3 py-2 font-bold whitespace-nowrap ${r.approved_price != null && r.approved_price !== r.requested_price ? 'text-amber-700' : 'text-emerald-700'}`}>
                      {fmt(effectiveApproved)}
                      {r.approved_price != null && r.approved_price !== r.requested_price && (
                        <span className="ml-1 text-[9px] bg-amber-100 text-amber-700 px-1 rounded">MOD</span>
                      )}
                    </td>
                    <td className={`px-3 py-2 font-semibold whitespace-nowrap ${diff!=null&&diff<0?'text-red-600':'text-emerald-700'}`}>
                      {diff!=null?`${diff>0?'+':''}₹${diff.toFixed(2)}`:'—'}
                    </td>
                    <td className={`px-3 py-2 whitespace-nowrap ${diff!=null&&diff<0?'text-red-600':'text-emerald-700'}`}>{fmtPct(r.normal_price,effectiveApproved)}</td>
